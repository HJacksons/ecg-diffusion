         ...
         for folder in ['train', 'test', 'validation']:
            for data_dir in self.data_dir:
                for filename in glob.glob(f'{data_dir}/{folder}/*.asc'):
                    ...
                    temp_df = pd.read_csv(filename, sep=" ", names=Synthetic_Dataset.header)
                    temp_tensor_in = torch.tensor(Synthetic_Dataset.convert_input(temp_df.iloc[:, 0]), dtype=torch.float32).unsqueeze(0)
                    temp_tensor_out = torch.tensor(Synthetic_Dataset.convert_input(temp_df.iloc[:, 1:9].values), dtype=torch.float32).t()
